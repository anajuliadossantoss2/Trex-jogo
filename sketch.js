...
function setup() {
  ...
  gameOver = createSprite(windowWidth/2,windowHeight -340);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyDown("space") && trex.y >= windowHeight -220) {
      trex.velocityY = -12;
    }
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(windowHeight -300, windowHeight -400));
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(windowWidth,windowHeight-203,10,40);
    //obstacle.debug = true;
